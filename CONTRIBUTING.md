...
2. Run `uv run --no-sync ruff format --check .`, `uv run --no-sync ruff check .`,
   `uv run --no-sync mypy src`, `uv run --no-sync pytest`,
   `uv run --no-sync python -m probezen.benchmark`, and `uv build`.
